module Opinion
	module ActsAsVoteable #:nodoc:
		...
		def self.included(base)
			base.extend Opinion::Base
			base.extend ClassMethods
		end

		module ClassMethods
			def acts_as_voteable
				has_many Opinion.configuration[:voteable_relationship_name],
				         :as => :voteable,
				         :dependent => :destroy,
				         :class_name => 'Opinion::Vote'

				include Opinion::ActsAsVoteable::InstanceMethods
				extend  Opinion::ActsAsVoteable::SingletonMethods
			end
		end
		...
		module SingletonMethods
			...
			def plusminus_tally(params = {})
				t = self.joins("LEFT OUTER JOIN #{Opinion::Vote.table_name} ON #{self.table_name}.id = #{Opinion::Vote.table_name}.voteable_id AND #{Opinion::Vote.table_name}.voteable_type = '#{self.name}'")
				t = t.order("plusminus_tally DESC")
				t = t.group(column_names_for_tally)
				t = t.select("#{self.table_name}.*")
				t = t.select("SUM(CASE #{Opinion::Vote.table_name}.vote WHEN #{quoted_true} THEN 1 WHEN #{quoted_false} THEN -1 ELSE 0 END) AS plusminus_tally")
				if params[:separate_updown]
					t = t.select("SUM(CASE #{Opinion::Vote.table_name}.vote WHEN #{quoted_true} THEN 1 WHEN #{quoted_false} THEN 0 ELSE 0 END) AS up")
					t = t.select("SUM(CASE #{Opinion::Vote.table_name}.vote WHEN #{quoted_true} THEN 0 WHEN #{quoted_false} THEN 1 ELSE 0 END) AS down")
				end
				t = t.select("COUNT(#{Opinion::Vote.table_name}.id) AS vote_count")
			end
			...
			# i.e. Posts.tally.where('votes.created_at > ?', 2.days.ago)
			def tally(*args)
				t = self.joins("LEFT OUTER JOIN #{Opinion::Vote.table_name} ON #{self.table_name}.id = #{Opinion::Vote.table_name}.voteable_id")
		...
		module InstanceMethods
			...
			def _votes_on
				self.send(Opinion.configuration[:voteable_relationship_name])
			end
